...
class UnicodeProcessor:
    ...
    def __call__(self, text_list: list[str]) -> tuple[np.ndarray, np.ndarray]:
        ...
        text_ids_lengths = np.array([len(text)
                                    for text in text_list], dtype=np.int64)
        text_ids = np.zeros(
            (len(text_list), text_ids_lengths.max()), dtype=np.int64)
...
class TextToSpeech:
    ...
    def sample_noisy_latent(
        ...
        latent_len = ((wav_len_max + chunk_size - 1) /
                      chunk_size).astype(np.int32)
        latent_dim = self.ldim * self.chunk_compress_factor
        noisy_latent = np.random.randn(
            bsz, latent_dim, latent_len).astype(np.float32)
    ...
    def _infer(
        ...
        dur_onnx, *_ = self.dp_ort.run(
            None, {"text_ids": text_ids,
                   "style_dp": style.dp, "text_mask": text_mask}
        )
...
def load_voice_style(voice_style_paths: list[str], verbose: bool = False) -> Style:
    ...
    for i, voice_style_path in enumerate(voice_style_paths):
        ...
        dp_data = np.array(voice_style["style_dp"]
                           ["data"], dtype=np.float32).flatten()
